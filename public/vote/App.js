var App = {
	...
	Firebase: {
		config: {
			...
			projectId: "android-masters-vote",
			...
			App.Firebase.database.ref("users/"+App.User.loggedUser.uid).once("value", function(data) {
				...
				App.Finalists.list.forEach(function(entry) {
					...
					if(entry.name == data.val().vote) 
						selected = " selected";
					$(container).append('<a data-name="'+entry.name+'" class="card'+ selected +'"><span class="app">'+entry.name+'</span><span class="team">'+entry.team+'</span></a>');
				});
			});
		},
		loadScore: function() {
